...
void GpuIndex::searchFromCpuPaged_(
    ...
    std::unique_ptr<HipEvent> eventPinnedCopyDone[2];

    // Execute completion events for the GPU buffers
    std::unique_ptr<HipEvent> eventGpuExecuteDone[2];
    ...
    while (cur3 < n) {
        ...
        if (cur2 != -1 && cur2 < n) {
            ...
            eventPinnedCopyDone[cur2BufIndex].reset(new HipEvent(copyStream));

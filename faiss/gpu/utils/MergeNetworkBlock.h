...
// Merge pairs of lists smaller than hipBlockDim_x (NumThreads)
template <
        int NumThreads,
        typename K,
        typename V,
...
inline __device__ void blockMergeSmall(K* listK, V* listV) {
    ...
    int mergeId = hipThreadIdx_x / L;

    // Which thread we are within the merge
    int tid = hipThreadIdx_x % L;
    ...
    if (AllThreads || (hipThreadIdx_x < N * L)) {
        K ka = listK[pos];
...
// Merge pairs of sorted lists larger than hipBlockDim_x (NumThreads)
template <
        int NumThreads,
        typename K,
        typename V,
...
#pragma unroll
    for (int loop = 0; loop < kLoopPerThread; ++loop) {
        int tid = loop * NumThreads + hipThreadIdx_x;
        int pos = L - 1 - tid;
        int stride = 2 * tid + 1;

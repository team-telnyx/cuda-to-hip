...
__global__ void incrementIndex(
        ...
        int k,
        idx_t increment) {
    for (idx_t i = hipBlockIdx_y; i < indices.getSize(0); i += hipGridDim_y) {
        for (int j = hipThreadIdx_x; j < k; j += hipBlockDim_x) {
            indices[i][idx_t(hipBlockIdx_x) * k + j] += hipBlockIdx_x * increment;
        }
    }
}
...
void runIncrementIndex(
    ...
    hipLaunchKernelGGL(incrementIndex, grid, dim3(block), 0, stream, indices, k, increment);
    HIP_TEST_ERROR();
}

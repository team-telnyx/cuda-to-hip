...
__global__ void pqScanPrecomputedInterleaved(
    ...
    auto queryId = hipBlockIdx_y;
    auto probeId = hipBlockIdx_x;
    ...
    int numWarps = hipBlockDim_x / kWarpSize;
...
#pragma unroll
            for (int j = 0; j < kUnroll; ++j) {
                LoadStore<LookupVecT>::store(
                        &smemV[i + j * hipBlockDim_x], vals[j]);
            }
        }

        // This is where we start loading the remainder that does not evenly
        // fit into kUnroll x hipBlockDim_x
        int remainder = limitVec * kWordSize;

        for (idx_t i = remainder + hipThreadIdx_x; i < numCodes; i += hipBlockDim_x) {
        ...
        idx_t limit = utils::roundDown(numCodes, kUnroll * hipBlockDim_x);

        idx_t i = hipThreadIdx_x;
        for (; i < limit; i += kUnroll * hipBlockDim_x) {
            LookupT vals[kUnroll];

#pragma unroll
            for (int j = 0; j < kUnroll; ++j) {
                vals[j] = term2Start[i + j * hipBlockDim_x];
            }

#pragma unroll
            for (int j = 0; j < kUnroll; ++j) {
                vals[j] = Math<LookupT>::add(
                        vals[j], term3Start[i + j * hipBlockDim_x]);
            }

#pragma unroll
            for (int j = 0; j < kUnroll; ++j) {
                smem[i + j * hipBlockDim_x] = vals[j];
            }
        }

        for (; i < numCodes; i += hipBlockDim_x) {
            smem[i] = Math<LookupT>::add(term2Start[i], term3Start[i]);
        }
    }
}
...
__global__ void pqScanPrecomputedMultiPass(
    ...
    if (hipThreadIdx_x < limit) {
        LoadCode32<NumSubQuantizers>::load(code32, codeList, hipThreadIdx_x);
    }
    ...
    // block-wide stride
    for (idx_t codeIndex = hipThreadIdx_x; codeIndex < limit;
         codeIndex += hipBlockDim_x) {
        // Prefetch next codes
        if (codeIndex + hipBlockDim_x < limit) {
            LoadCode32<NumSubQuantizers>::load(
                    nextCode32, codeList, codeIndex + hipBlockDim_x);
        }
...
#define RUN_INTERLEAVED(BITS_PER_CODE, CODE_DIST_T)     \
    do {                                                \
        hipLaunchKernelGGL(HIP_KERNEL_NAME(pqScanPrecomputedInterleaved<uint8_t, BITS_PER_CODE, CODE_DIST_T>),  \
        grid, block, 0, stream, queries, precompTerm1, precompTerm2T, precompTerm3T, ivfListIds,    \
        listCodes.data(), listLengths.data(), prefixSumOffsets, allDistances); \
    } while (0)
...
void runPQScanMultiPassPrecomputed(
    ...
    constexpr idx_t kMaxQueryTileSize = 65536; // typical max hipGridDim_y

...
__global__ void runUpdateListPointers(
        ...
        void** listCodes,
        void** listIndices) {
    idx_t i = idx_t(hipBlockIdx_x) * hipBlockDim_x + hipThreadIdx_x;
...
__global__ void ivfIndicesAppend(
        ...
        IndicesOptions opt,
        void** listIndices) {
    idx_t vec = idx_t(hipBlockIdx_x) * hipBlockDim_x + hipThreadIdx_x;
...
void runIVFIndicesAppend(
    ...
    if (opt != INDICES_CPU && opt != INDICES_IVF) {
        ...
        hipLaunchKernelGGL(ivfIndicesAppend, dim3(blocks), dim3(threads), 0, stream, 
...
__global__ void ivfFlatAppend(
    ...
    idx_t i;
    for (i = hipThreadIdx_x; i < limit; i += hipBlockDim_x) {
...
__global__ void ivfpqAppend(
        ...
        void** listCodes) {
    idx_t encodingToAdd = idx_t(hipBlockIdx_x) * hipBlockDim_x + hipThreadIdx_x;
...
void runIVFPQAppend(
    ...
    hipLaunchKernelGGL(ivfpqAppend, dim3(threads), dim3(blocks), 0, stream, 
...
__global__ void sqEncode(
    ...
    for (idx_t d = hipThreadIdx_x; d < vecs.getSize(1); d += hipBlockDim_x) {
...
void runSQEncode(
    ...
    hipLaunchKernelGGL(sqEncode, dim3(blocks), dim3(threads), 0, stream, vecs, encodedVecs, codec);
}
...
__global__ void ivfInterleavedAppend(
    ...
    int warpsPerBlock = hipBlockDim_x / kWarpSize;

    // Each block is dedicated to a separate list
    idx_t listId = uniqueLists[hipBlockIdx_x];

    // The vecs we add to the list are at indices [vBUL[vecIdStart],
    // vBUL[vecIdEnd])
    idx_t vecIdStart = uniqueListVectorStart[hipBlockIdx_x];
    // uLVS is explicitly terminated for us with one more than the number of
    // blocks that we have
    idx_t vecIdEnd = uniqueListVectorStart[hipBlockIdx_x + 1];
    ...
    auto listVecStart = uniqueListStartOffset[hipBlockIdx_x];
...
#define RUN_APPEND(ENCODE_T, ENCODE_BITS, DATA)     \
    do {                                            \
        ...
        dim3 block(128);                            \
        hipLaunchKernelGGL(HIP_KERNEL_NAME(ivfInterleavedAppend<ENCODE_T, ENCODE_BITS>), \
                        grid, block, 0, stream,     \
                        uniqueLists,                \

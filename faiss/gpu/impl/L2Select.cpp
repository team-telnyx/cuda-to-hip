...
#pragma unroll
    ...
    if (endRow) {
        for (idx_t row = rowStart; row < productDistances.getSize(0); ++row) {
            for (idx_t col = hipThreadIdx_x; col < productDistances.getSize(1);
                 col += hipBlockDim_x) {
...
#define L2_KERNEL(INDEX_T, BLOCK, NUM_WARP_Q, NUM_THREAD_Q)   \
    hipLaunchKernelGGL(HIP_KERNEL_NAME(l2SelectMinK<INDEX_T, T, NUM_WARP_Q, NUM_THREAD_Q, BLOCK>), \

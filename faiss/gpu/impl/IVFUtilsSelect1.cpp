...
__global__ void pass1SelectLists(
    ...
    for (IndexT queryId = hipBlockIdx_y; queryId < prefixSumOffsets.getSize(0);
         queryId += hipGridDim_y) {
        ...
        auto sliceId = hipBlockIdx_x;
        auto numSlices = hipGridDim_x;
...
#define RUN_PASS(INDEX_T, BLOCK, NUM_WARP_Q, NUM_THREAD_Q, DIR)         \
    do {                                                                \
        hipLaunchKernelGGL(HIP_KERNEL_NAME(pass1SelectLists<INDEX_T, BLOCK, NUM_WARP_Q, NUM_THREAD_Q, DIR>), \

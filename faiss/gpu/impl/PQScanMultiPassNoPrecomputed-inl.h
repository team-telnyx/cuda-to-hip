...
#pragma unroll
            ...
            for (int i = remainder + hipThreadIdx_x; i < numCodes;
                 i += hipBlockDim_x) {
                smem[i] = codes[i];
            }
        } else {
            ...
            int limit = utils::roundDown(numCodes, kUnroll * hipBlockDim_x);
...
#pragma unroll
                for (int j = 0; j < kUnroll; ++j) {
                    vals[j] = codes[i + j * hipBlockDim_x];
                }
...
#define RUN_INTERLEAVED(BITS_PER_CODE, CODE_DIST_T)            \
    do {                                                       \
        hipLaunchKernelGGL(HIP_KERNEL_NAME(pqScanInterleaved<uint8_t, BITS_PER_CODE, CODE_DIST_T>), \
                        grid, block, 0, stream,                \
                        queries,                               \
                        pqCentroidsInnermostCode,              \
                        coarseIndices,                         \

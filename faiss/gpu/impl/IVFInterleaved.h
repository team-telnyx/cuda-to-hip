...
__global__ void ivfInterleavedScan(
    ...
    for (idx_t queryId = hipBlockIdx_y; queryId < queries.getSize(0);
         queryId += hipGridDim_y) {
        int probeId = hipBlockIdx_x;
...
#define IVFINT_RUN(CODEC_TYPE, METRIC_TYPE, THREADS, NUM_WARP_Q, NUM_THREAD_Q) \
    do {                                                                       \
        ...
        if (useResidual) {                                                     \
            hipLaunchKernelGGL(HIP_KERNEL_NAME(ivfInterleavedScan<             \
                    CODEC_TYPE,                                                \
                    METRIC_TYPE,                                               \
                    THREADS,                                                   \
                    NUM_WARP_Q,                                                \
                    NUM_THREAD_Q,                                              \
                    true>), grid, dim3(THREADS), codec.getSmemSize(dim), stream,     \
                    queries,                                                   \
                    residualBase,                                              \
                    listIds,                                                   \
                    listData.data(),                                           \
            ...
            hipLaunchKernelGGL(HIP_KERNEL_NAME(ivfInterleavedScan<             \
                    ...
                    NUM_WARP_Q,                                                \
                    NUM_THREAD_Q,                                              \
                    false>), grid, dim3(THREADS), codec.getSmemSize(dim), stream,    \
                    queries,                                                   \
                    residualBase,                                              \
                    listIds,                                                   \
                    listData.data(),                                           \

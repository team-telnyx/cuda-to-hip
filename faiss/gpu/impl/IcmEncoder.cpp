...
__global__ void runIcmEncodeStep(
    ...
    int id = hipBlockIdx_x;    // each block takes care of one vector
    int code = hipThreadIdx_x; // each thread takes care of one possible code
...
__global__ void runEvaluation(
        ...
        int dims) {
    int id = hipBlockIdx_x; // each block takes care of one vector
    int d = hipThreadIdx_x; // each thread takes care of one dimension
    float acc = 0.0f;

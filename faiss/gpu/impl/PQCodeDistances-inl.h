...
__global__ void __launch_bounds__(288, 3) pqCodeDistances(
    ...
    bool isLoadingThread = hipThreadIdx_x >= codesPerSubQuantizer;
    int loadingThreadId = hipThreadIdx_x - codesPerSubQuantizer;
    ...
    auto code = hipThreadIdx_x;
    auto subQuantizer = hipBlockIdx_y;
...
#pragma unroll
    ...
    auto startQueryId = idx_t(hipBlockIdx_x) * queriesPerBlock;
    ...
    for (int query = 0; query < numQueries; ++query) {
        ...
        for (int i = hipThreadIdx_x; i < DimsPerSubQuantizer; i += hipBlockDim_x) {
            smemQuery[i] = querySubQuantizer[i];
        }

        // Load list of coarse centroids found
        for (int i = hipThreadIdx_x; i < coarseIndices.getSize(1);
             i += hipBlockDim_x) {
        ...
        if (isLoadingThread) {
            for (int i = loadingThreadId; i < DimsPerSubQuantizer;
                 i += hipBlockDim_x - codesPerSubQuantizer) {
...
__global__ void pqResidualVector(
    ...
    auto queryId = hipBlockIdx_x;
    auto centroidId = hipBlockIdx_y;

    idx_t realCentroidId = coarseIndices[queryId][centroidId];

    for (int dim = hipThreadIdx_x; dim < queries.getSize(1); dim += hipBlockDim_x) {
...
void runPQResidualVector(
        ...
        bool l2Residual,
        hipStream_t stream) {
    // hipBlockDim_y is limited by nprobe
...
__global__ void pqDistanceIPCorrection(
        ...
        int numSubQ) {
    int centroid = hipBlockIdx_x;
    int query = hipBlockIdx_y;
    ...
    for (int i = hipThreadIdx_x; i < codeDistances.getSize(2); i += hipBlockDim_x) {
        base[i] += d;
    }
}
...
void runPQDistanceIPCorrection(
    ...
    hipLaunchKernelGGL(pqDistanceIPCorrection, grid, dim3(block), 0, stream, 
    ...
    do {                                                                 \
        if (useFloat16Lookup) {                                          \
            auto outCodeDistancesT = outCodeDistances.toTensor<half>();  \
                                                                         \
            hipLaunchKernelGGL(HIP_KERNEL_NAME(pqCodeDistances<half, CentroidT, DIMS, L2>),                   \
